...
markers_list <- c(markers_list, 'neural_markers_list')
# 添加到 markers_list 变量中
markers_list <- c(markers_list, 'neural_markers_list')
...
myeloids_markers_list2 = list(
  ...
  Macro = c("APOC1","HLA-DRB5","C1QA","C1QB"),
  RTMs = c("THBS1"),#Resident tissue macrophages
  Lam = c("APOE"),#Lipid associated macrophages
  Mono = c("LYZ","HLA-DRB1","TIMP1",
                'CD14','FCGR3A',
                "S100A11","CXCL8","IL1B","PTGS2","S100A9","S100A8","MMP19"),
...
fibro_marker.genes = c('COL6A3',"CDH11",
...
Tcells_marker.genes = c("XCL1","XCL2","FCGR3A",'KLRD1',"KLRF1",'KLRC1','KLRB1',
...
av <-AggregateExpression(sce.all.int ,
                         #group.by = "RNA_snn_res.0.5"  ,
                         assays = "RNA") 
av=as.data.frame(av[[1]])
head(av) # 可以看到是整数矩阵
av=av[ rowSums(av>0) > 1 ,]
write.csv(av,file = 'AggregateExpression.csv')


pdf( 'balloonplot-by-orig.ident.pdf' ,width = 10,height = 10)
gplots::balloonplot(
  table( Idents(sce.all.int) ,sce.all.int$orig.ident)
)
dev.off()
