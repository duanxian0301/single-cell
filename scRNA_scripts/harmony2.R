run_harmony <- function(input_sce){
  ...
  seuratObj <- RunUMAP(seuratObj, 
                    dims = 1:20,       # 尝试增加PC数量（原15）
                    reduction = "harmony",
                    n.neighbors = 30,  # 调整邻居数（默认30，可尝试10-50）
                    min.dist = 0.3,    # 调整点间距（默认0.3，越小聚类越紧凑）
                    metric = "cosine") # 尝试不同距离度量
  ...
  p1_dim=plot_grid(ncol = 3, DimPlot(input_sce.all, reduction = "umap", group.by = "RNA_snn_res.0.01") + 
                     ...
                     ggtitle("louvain_0.2"))
  ggsave(plot=p1_dim, filename="2-harmony/Dimplot_diff_resolution_low.pdf",width = 14)
  ...
  p1_dim=plot_grid(ncol = 3, DimPlot(input_sce.all, reduction = "umap", group.by = "RNA_snn_res.0.8") + 
                     ...
                     ggtitle("louvain_0.3"))
  ggsave(plot=p1_dim, filename="2-harmony/Dimplot_diff_resolution_high.pdf",width = 18)
  
  p2_tree=clustree(input_sce.all@meta.data, prefix = "RNA_snn_res.")
  ggsave(plot=p2_tree, filename="2-harmony/Tree_diff_resolution.pdf")
  table(input_sce.all@active.ident) 
  saveRDS(input_sce.all, "2-harmony/sce.all_int.rds")
